		...
		while 1:
				...
				for i in range(int(g_config["tseg"]/g_config["tavg"])):
						...
						try:
								...
								del_get = jobj['message_stats']['publish']
								qlen = jobj['messages']
								...
								if self.m_del_get!=del_get:
									...
									if last_failed:
										last_failed = False
										smsstr = "队列%s(len:%d)现在正常插入.[109]"%(self.m_queue,qlen)
				...
				if 0!=qlen and not is_get and not last_failed:
					...
					smsstr = "队列%s(len:%d)已经有 %dmin 没有插入数据.[109]"%(self.m_queue,qlen,int(g_config["tseg"]/60))
					print smsstr
					g_rmq_mgr.send(smsstr)
					last_failed = True
					pass #sms send.
